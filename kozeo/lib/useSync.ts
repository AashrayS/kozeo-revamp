...
import { useEffect , useCallback, useRef , useState, Dispatch, SetStateAction } from 'react'
import {
	Editor,
...
// Generic hook to persist identity if shallow-equal
...
export function useSync(opts: UseSyncOptions & TLStoreSchemaOptions): RemoteTLStoreWithStatus {
	...
	useEffect(() => {
		...
		const client = new TLSyncClient({
			...
			onSyncError(reason : TLSyncErrorCloseEventReason) {
			...
			onAfterConnect(_: unknown, { isReadonly }: { isReadonly: boolean })  {
				transact(() => {
