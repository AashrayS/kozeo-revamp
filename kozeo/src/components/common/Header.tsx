...
export default function Header({
  ...
  return (
    <>
      <header
        className={`w-full px-6 py-4 Z-40 flex justify-between border-b-1 border-forge-line items-center relative theme-transition ${
          theme === "light"
            ? "bg-forge-bg text-forge-ink"
            : "bg-forge-bg from-black  text-forge-ink"
        }`}
      >
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className={`relative flex items-center gap-2 px-4 py-2 rounded-sm transition-colors ${
              theme === "light"
                ? "text-forge-ink hover:bg-forge-bg"
                : "text-forge-ink hover:bg-forge-bg-raised"
            }`}
          >
            ...
            {/* Notification Badge */}
            {unreadCount > 0 && (
              <span className="absolute -top-1 -right-1 w-3 h-3 bg-forge-ember rounded-sm animate-pulse-slow" />
            )}
          </button>
